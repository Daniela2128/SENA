...
public class FichaDao implements MeFicha{
  ...
  @Override
    ...
    public boolean registroficha(Ficha fi) {
       ...
       JOptionPane.showMessageDialog(null,"No se puede registrar la ficha" +e.getMessage());
        }
        return false;
    }
    ...
    public boolean actualizarficha(Ficha fi) {
        String sql="update ficha set codficha='"+fi.getCodficha()+"',cantapre='"+fi.getCantapre()+"',codpro='"+fi.getCodpro()+"',codaprendiz='"+fi.getCodaprendiz();
                try{
                con=cn.getConnection();
                ps=con.prepareStatement(sql);
                ps.executeUpdate();
