...
public class AprendizDao implements MeAprendiz {
    ...
    public boolean actualizarape(Aprendiz ape) {
        String sql="update aprendiz set Docu='"+ap.getDocu()+"',Nombreap='"+ap.getNombreap()+"',Apellidoap='"+ap.getApellidoap()+"',Correoap='"+ap.getCorreoap()+"',Telea='"+ap.getTeleap()+"'where id="+ap.getid();
                try{
                con=cn.getConnection();
                ps=con.prepareStatement(sql);
                ps.executeUpdate();
